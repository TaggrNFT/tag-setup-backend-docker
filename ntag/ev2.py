...
class AuthenticateEV2:
    ...
    @classmethod
    def get_init_packet(cls, key_no: bytes) -> bytes:
        """
    ...
    def init(self, key_no: bytes) -> bytes:
        return AuthenticateEV2.get_init_packet(key_no)
...
class CryptoComm:
    ...
    def serialize(self) -> dict:
        return {"k_ses_auth_mac": self.k_ses_auth_mac.hex(),
                "k_ses_auth_enc": self.k_ses_auth_enc.hex(),
                "ti": self.ti.hex(),
                "cmd_counter": self.cmd_counter,
                "pdcap2": self.pdcap2.hex(),
                "pcdcap2": self.pcdcap2.hex()}

    @classmethod
    def unserialize(cls, data: dict) -> 'CryptoComm':
        k_ses_auth_mac = binascii.unhexlify(data["k_ses_auth_mac"])
        k_ses_auth_enc = binascii.unhexlify(data["k_ses_auth_enc"])
        ti = binascii.unhexlify(data["ti"])
        cmd_counter = data["cmd_counter"]
        pdcap2 = binascii.unhexlify(data["pdcap2"])
        pcdcap2 = binascii.unhexlify(data["pcdcap2"])

        return CryptoComm(k_ses_auth_mac, k_ses_auth_enc, ti=ti, cmd_counter=cmd_counter, pdcap2=pdcap2, pcdcap2=pcdcap2)
